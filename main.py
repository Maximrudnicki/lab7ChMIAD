...
class SecondWindow(FirstWindow):
    ...
    def __init__(self, master):
        ...
        self.sortBtn = Button(frame1, text="Clear List", command=self.clear_list)
        ...
        self.R1.pack(fill=X)
        self.R2 = Radiobutton(master, text="By year", variable=var1, value='2', command=self.sort_by_year)
        self.R2.pack(fill=X)
        self.R3 = Radiobutton(master, text="By duration", variable=var1, value='3', command=self.sort_by_duration)
        self.R3.pack(fill=X)
        self.R4 = Radiobutton(master, text="By producer", variable=var1, value='4', command=self.sort_by_producer)
        self.R4.pack(fill=X)
        self.quickBtn = Button(master, text="Sort", command=self.sort)
    ...
    def write_to_file(self):
        ...
        write_file.close()
        try:
            filename = self.txt.get() + str(".ser")
            int(self.txt1.get())
            int(self.txt2.get())
            str(self.txt3.get())

            with open(filename, "ab") as file:
                pickle.dump(f"{self.txt.get()}, {self.txt1.get()}, {self.txt2.get()}, {self.txt3.get()}\n", file)
        except ValueError:
